...
fn selfhost_preprocessed_kpsc_main_stage3_forth_is_native_backend_clean() {
    ...
    let flat_selfhost = preprocess_pascal_file("selfhost/kpsc_main.pas");
    let stage1_src = run_compiler(&flat_selfhost);
    let (_stage1_work_dir, stage1_bin_path) = build_native_forth_binary(
        "preprocessed-selfcompile-stage1-native-clean-bin",
        &stage1_src,
    );
    ...
    let stage2 = run_native_binary_with_input(&stage1_bin_path, &encoded);
    let (_stage2_work_dir, stage2_bin_path) =
        build_native_forth_binary("preprocessed-selfcompile-stage2-native-clean-bin", &stage2);
...
fn selfhost_preprocessed_kpsc_main_fresh_stage2_compiler_handles_generic_arithmetic_program() {
    ...
    let stage1_src = run_compiler(&flat_selfhost);
    let (_stage1_work_dir, stage1_bin_path) = build_native_forth_binary(
        "preprocessed-selfcompile-stage1-fresh-arith-bin",
        &stage1_src,
    );
...
#[test]
#[ignore = "extension-only on standard Pascal branch"]
...
fn selfhost_stage2_string_feature_progression() {
    ...
    let stage2_forth = cached_stage2_forth();
    let (_work_dir, stage2_bin) =
        build_native_forth_binary("stage2-string-progression-bin", &stage2_forth);
...
begin
  ...
  WriteLn(1)
    ...
    for (label, src, expected) in cases {
        ...
        let emitted_forth = run_native_binary_with_input(&stage2_bin, &encoded);
        let got = run_native_forth(
            &format!("stage2-string-progression-{label}"),
            &emitted_forth,
        );
        assert_eq!(
            got.trim_end(),
            expected,
            "stage2 string progression failed: {label}"
        );
    }
}
...
fn selfhost_stage3_matches_restored_sample_outputs() {
    ...
    let stage3_forth = cached_stage3_forth();
    let (_work_dir, stage3_bin) =
        build_native_forth_binary("stage3-restored-samples-bin", &stage3_forth);
