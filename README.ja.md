...
Pascal/0 を基礎に、32bit Forth VM 向けの Standard Pascal 寄りコアと、ブランチごとの拡張を持つ実装です。

## 主な機能

実際の機能面はブランチに依存します。以下は公開リポジトリ全体としての共通方向であり、制約や追加機能は各ブランチ側の文書で管理します。
...
標準 Pascal 系ブランチの前提条件は `STANDARD_PASCAL_PRECONDITIONS.md` に記録しています。
...
- 実際の言語表面はブランチごとに異なります。この README では共通事項のみを述べ、ブランチ固有の方針は `STANDARD_PASCAL_PRECONDITIONS.md` のような個別文書へ切り分けます。
